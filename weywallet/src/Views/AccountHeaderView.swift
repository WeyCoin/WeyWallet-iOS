...
private let smallFontSize: CGFloat = 26.0
...
class AccountHeaderView : UIView, GradientDrawable, Subscriber {
    ...
    init(store: Store) {
        ...
        self.exchangeRate = Rate(code: "WAE", name: "WeyCoin", rate: 1.0)
        
        let placeholderAmount = Amount(amount: 0, rate: self.exchangeRate!, maxDigits: store.state.maxDigits)
    ...
    private func addConstraints() {
        ...
        primaryBalance.constrain([
            primaryBalance.constraint(.top, toView: self, constant: 90.0),
            primaryBalance.constraint(.left, toView: self, constant: 14.0)
        ])
