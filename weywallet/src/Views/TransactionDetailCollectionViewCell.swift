...
class TransactionDetailCollectionViewCell : UICollectionViewCell {
    ...
    func set(transaction: Transaction, isBtcSwapped: Bool, rate: Rate, rates: [Rate], maxDigits: Int) {
        ...
        self.rate = Rate(code: "WAE", name: "WeyCoin", rate: 1.0)
        
        timestamp.text = transaction.longTimestamp
        amount.text = String(format: transaction.direction.amountFormat, "\(transaction.amountDescription(isBtcSwapped: isBtcSwapped, rate: self.rate!, maxDigits: maxDigits))")
        address.text = transaction.detailsAddressText
        status.text = transaction.status
        comment.text = transaction.comment
        amountDetails.text = transaction.amountDetails(isBtcSwapped: isBtcSwapped, rate: self.rate!, rates: [self.rate!], maxDigits: maxDigits)
